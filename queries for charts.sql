...
from (
         ...
         from (
                  ...
                  from (
                           ...
                           group by executiondate
                           order by executiondate
                       ) as subq2 full outer join (
                      select ROW_NUMBER () OVER (ORDER BY executiondate) - (
                          select (count(*)+1)/2.0
                          from (
                                   select distinct executiondate
                                   from preview p
                               ) as subq2
                      ) as x_a, executiondate
                      from (
                               select distinct executiondate
                               from preview p
                           ) as subq1
                  ) as subq3
                                                  on subq2.executiondate = subq3.executiondate
                  where subq2.executiondate is not null or subq3.executiondate is not null
              ) as subq4
     ) as sub6, (
